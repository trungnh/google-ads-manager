            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-title">Chi tiết chiến dịch Performance Max: <?= esc($campaignDetails['name']) ?></h4>
                        <div>
                ...
                <div class="card-body">
                    ...
                    <div class="card mt-4">
                        <div class="card-header">
                            <h5 class="card-title">Danh sách nhóm thành phần (Asset Groups)</h5>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td colspan="10" class="text-center">Không có nhóm thành phần nào</td>
                                        </tr>
                                        <?php else: ?>
                                            <?php foreach ($assetGroups as $assetGroup): ?>
                                            <tr>
                                                <td><?= esc($assetGroup['asset_group_id']) ?></td>
                                                <td><?= esc($assetGroup['name']) ?></td>
                                                <td>
                                                    <?php if ($assetGroup['status'] == 'ENABLED'): ?>
                                                        <span class="badge bg-success">Đang chạy</span>
                                                    <?php elseif ($assetGroup['status'] == 'PAUSED'): ?>
                                                        <span class="badge bg-warning">Tạm dừng</span>
                                                    <?php else: ?>
                                                        <span class="badge bg-secondary"><?= esc($assetGroup['status']) ?></span>
                                                    <?php endif; ?>
                                                </td>
                                                <td><?= number_format($assetGroup['cost'], 2) ?></td>
                                                <td><?= number_format($assetGroup['conversions'], 2) ?></td>
                                                <td><?= number_format($assetGroup['cost_per_conversion'], 2) ?></td>
                                                <td><?= number_format($assetGroup['ctr'] * 100, 2) ?>%</td>
                                                <td><?= number_format($assetGroup['clicks']) ?></td>
                                                <td><?= number_format($assetGroup['average_cpc'], 2) ?></td>
                                                <td>
                                                    <a href="<?= base_url('campaign-details/asset-group/' . $account['customer_id'] . '/' . $campaignDetails['campaign_id'] . '/' . $assetGroup['asset_group_id']) ?>" class="btn btn-sm btn-primary">

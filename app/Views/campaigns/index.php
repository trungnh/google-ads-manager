                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div>
                                ...
                                <a href="<?= base_url('adsaccounts/settings/' . $account['customer_id']) ?>"
                                    class="btn btn-sm btn-primary mb-2 mb-md-0">
                                    <i class="fas fa-cog"></i> Settings
                                </a>
                                <a href="<?= base_url('campaignschedules/' . $account['customer_id']) ?>"
                                    class="btn btn-sm btn-info ml-2 mb-2 mb-md-0">
                            ...
                            <div class="text-end">
                                <select class="form-select" id="accountSelector" style="width: 300px;"
                                    ...
                                    <?php foreach ($accounts as $acc): ?>
                                        <option value="<?= base_url('campaigns/index/' . $acc['customer_id']) ?>"
                                            <?= $acc['customer_id'] == $account['customer_id'] ? 'selected' : '' ?>>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="chartModalLabel">Biểu đồ chiến dịch</h5>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <div class="btn-group" role="group">
                            ...
                            <label class="btn btn-outline-primary" for="interval5m">5 phút</label>

                            <input type="radio" class="btn-check" name="chartInterval" id="interval30m" value="30"
                                autocomplete="off">
                            <label class="btn btn-outline-primary" for="interval30m">30 phút</label>
                        </div>
                    </div>
                    <div class="col-md-6 text-end">
                        <input type="date" class="form-control d-inline-block w-auto" id="chartDate"
                            value="<?= date('Y-m-d') ?>">
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function initializeChartButtons() {
            $('.btn-chart-campaign').off('click').on('click', function (e) {
                e.preventDefault();
                const btn = $(this);
                activeCampaignIdForChart = btn.data('campaign-id');
                activeCampaignNameForChart = btn.data('campaign-name');
                const customerId = btn.data('customer-id');

                $('#chartModalLabel').text(`Biểu đồ chiến dịch: ${activeCampaignNameForChart}`);

                // Set default date to today and 5m
                $('#chartDate').val(new Date().toISOString().split('T')[0]);
                $('#interval5m').prop('checked', true);

                // Display modal
                let myModal = new bootstrap.Modal(document.getElementById('chartModal'));
                myModal.show();

                loadChartData(customerId, activeCampaignIdForChart);
            });
        }

        // Refresh chart on interval or date change
        $('input[name="chartInterval"], #chartDate').on('change', function () {
            if (activeCampaignIdForChart) {
                loadChartData(currentCustomerId, activeCampaignIdForChart);
            }
        });

        function loadChartData(customerId, campaignId) {
            const interval = $('input[name="chartInterval"]:checked').val();
            const date = $('#chartDate').val();

            $.ajax({
                url: `/campaigns/chart/${customerId}/${campaignId}`,
                method: 'GET',
                data: {
                    interval: interval,
                    date: date
        ...
        function renderChart(data) {
            ...
            const costData = data.map(item => parseFloat(item.cost));
            const cpaData = data.map(item => parseFloat(item.cpa));
            const conversionsData = data.map(item => parseFloat(item.conversions));
            const cpcData = data.map(item => parseFloat(item.cpc));

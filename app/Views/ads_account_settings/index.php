            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="text-end mb-3">
                        <select class="form-select" id="accountSelector" style="width: 300px;"
                            onchange="window.location.href=this.value">
                            <?php foreach ($accounts as $acc): ?>
                                <option value="<?= base_url('adsaccounts/settings/' . $acc['customer_id']) ?>"
                ...
                <div class="card-body">
                    <form id="settingsForm" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="cost_threshold"
                                name="cost_threshold"
                                value="<?= isset($settings['cost_threshold']) ? $settings['cost_threshold'] : '' ?>">
                            <small class="form-text text-muted">
                                <i>Khi chi tiêu vượt quá ngưỡng này, chiến dịch sẽ check các điều kiện để Tắt hoặc Tăng
                                    ngân sách</i>
                            </small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="increase_budget"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="cpa_threshold"
                                name="cpa_threshold"
                        ...
                        <div class="mb-3">
                            <label for="extended_cpa_threshold" class="form-label">Ngưỡng CPA giữa 2 lần chuyển
                                đổi</label>
                            <input type="number" step="0.01" class="form-control" id="extended_cpa_threshold"
                                name="extended_cpa_threshold"
                                value="<?= isset($settings['extended_cpa_threshold']) ? $settings['extended_cpa_threshold'] : '' ?>">
                            <small class="form-text text-muted">
                                <i>Nếu chi tiêu thêm hoặc CPA từ lần ra đơn gần nhất > ngưỡng này thì tạm dừng chiến
                                    dịch
                        ...
                        <div class="mb-3 mt-5">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="use_roas_threshold"
                                    name="use_roas_threshold" <?= isset($settings['use_roas_threshold']) && $settings['use_roas_threshold'] ? 'checked' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="roas_threshold"
                                name="roas_threshold"
                        ...
                        <div class="mb-3">
                            <label for="exclude_campaign_ids" class="form-label">Loại trừ chiến dịch (ID chiến
                                dịch)</label>
                            <input type="text" class="form-control" id="exclude_campaign_ids"
                                name="exclude_campaign_ids"
                                value="<?= isset($settings['exclude_campaign_ids']) ? $settings['exclude_campaign_ids'] : '' ?>">
                            <small class="form-text text-muted">
                                <i>Phần tự động tối ưu sẽ <strong>bỏ qua</strong> các chiến dịch này khi chạy tự động.
                                    Điền <strong>ID chiến dịch</strong>, phân cách bằng dấu ","</i>
                            </small>
                        </div>
                        <hr class="my-4">
                        ...
                        <div id="pancake_settings" class="mb-4"
                            style="display: <?= isset($settings['use_pancake']) && $settings['use_pancake'] ? 'block' : 'none' ?>">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="pancake_product_id" class="form-label">Mã sản phẩm
                                    (product_display_id)</label>
                                <input type="text" class="form-control" id="pancake_product_id"
                            ...
                            <div class="mb-3">
                                <label style="display: block;" for="pancake_exclude_tags" class="form-label">Loại trừ
                                    thẻ đơn hàng</label>
                                <div class="input-group mb-3" style="display: none;">
                                    <input type="text" class="form-control" id="pancake_exclude_tags"
                                        name="pancake_exclude_tags"
                                        value="<?= isset($settings['pancake_exclude_tags']) ? $settings['pancake_exclude_tags'] : '' ?>"
                                        readonly>

                                    <!-- Thêm hidden input để đảm bảo giá trị được gửi đi -->
                                    <input type="hidden" id="pancake_exclude_tags_hidden"
                                        name="pancake_exclude_tags_hidden"
                                        value="<?= isset($settings['pancake_exclude_tags']) ? $settings['pancake_exclude_tags'] : '' ?>">
                                    <button class="btn btn-outline-secondary" type="button" id="load_tags_button">Load
                                        các thẻ đơn hàng</button>
                                </div>
                                <small class="form-text text-muted">
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="pancake_use_usd"
                                        name="pancake_use_usd" <?= isset($settings['pancake_use_usd']) && $settings['pancake_use_usd'] ? 'checked' : '' ?>>
                            ...
                            <div class="mb-3" id="pancake_usd_rate_container"
                                style="display: <?= isset($settings['pancake_use_usd']) && $settings['pancake_use_usd'] ? 'block' : 'none' ?>">
                                <label for="pancake_usd_rate" class="form-label">Tỷ giá USD/VND</label>
                                <input type="number" step="1" class="form-control" id="pancake_usd_rate"
                                    name="pancake_usd_rate"
                        ...
                        <div id="gsheet_settings" class="mb-4"
                            ...
                            <div class="alert alert-info py-2 px-3 mb-3 d-flex align-items-center" style="font-size: 0.9rem;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-info-circle-fill me-2 flex-shrink-0" viewBox="0 0 16 16">
                            ...
                            <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="ggsheet_filename" class="form-label">Tên File Google Sheet <span
                                                    class="text-danger">*</span></label>
                                            <div class="input-group">
                                                <input type="hidden" id="ggsheet_id" name="ggsheet_id" value="<?= isset($settings['ggsheet_id']) ? $settings['ggsheet_id'] : '' ?>">
                                                <input type="text" class="form-control" id="ggsheet_filename"
                                                    value=""
                                                    placeholder="Chọn file Google Sheet..." readonly>
                                                <button class="btn btn-outline-primary btn-select-sheet" type="button" data-target-id="ggsheet_id" data-target-filename-id="ggsheet_filename" data-target-name="ggsheet_name">
                                                    Chọn File
                                                </button>
                                            </div>
                                            <div class="form-text">Tên file Google Sheet đã chọn</div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="ggsheet2_filename" class="form-label">Tên File Google Sheet 2</label>
                                            <div class="input-group">
                                                <input type="hidden" id="ggsheet2_id" name="ggsheet2_id" value="<?= isset($settings['ggsheet2_id']) ? $settings['ggsheet2_id'] : '' ?>">
                                                <input type="text" class="form-control" id="ggsheet2_filename"
                                                    value=""
                                                    placeholder="Chọn file Google Sheet 2..." readonly>
                                                <button class="btn btn-outline-primary btn-select-sheet" type="button" data-target-id="ggsheet2_id" data-target-filename-id="ggsheet2_filename" data-target-name="ggsheet2_name">
                                                    Chọn File
                                                </button>
                                            </div>
                                            <div class="form-text">Tên file Google Sheet 2 đã chọn</div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="gsheet_date_col"
                                            name="gsheet_date_col"
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="gsheet_phone_col"
                                            name="gsheet_phone_col"
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="gsheet_value_col"
                                            name="gsheet_value_col"
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="gsheet_campaign_col"
                                            name="gsheet_campaign_col"
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="default_paused_campaigns"
                                    name="default_paused_campaigns" <?= isset($settings['default_paused_campaigns']) && $settings['default_paused_campaigns'] ? 'checked' : '' ?>>
                                <label class="form-check-label" for="default_paused_campaigns">Default load chiến dịch
                                    đã tạm dừng</label>
                            </div>
                            <small class="form-text text-muted">
                                <i>Khi bật, ở trang dánh sách chiến dịch của tài khoản, hệ thống sẽ tự động load các
                                    chiến dịch đã tạm dừng</i>
                            </small>
                        </div>
                        <div class="mb-3">
...
<script>
    ...
    function createPicker(targetId, targetFilenameId, targetName) {
        ...
        var picker = new google.picker.PickerBuilder()
            ...
            .setCallback(function(data) {
                if (data[google.picker.Response.ACTION] == google.picker.Action.PICKED) {
                    ...
                    // Ghi ID và Tên File vào ô tương ứng
                    $('#' + targetId).val(docId);
                    $('#' + targetFilenameId).val(docName);
    ...
    // Lấy tên file Google Sheet dựa trên ID khi tải trang
    function fetchSpreadsheetTitle(docId, targetFilenameId) {
        if (!googleAccessToken || !docId) return;
        $.ajax({
            url: 'https://sheets.googleapis.com/v4/spreadsheets/' + docId + '?fields=properties.title',
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + googleAccessToken
            },
            success: function(response) {
                if (response && response.properties && response.properties.title) {
                    $('#' + targetFilenameId).val(response.properties.title);
                } else {
                    $('#' + targetFilenameId).val(docId);
                }
            },
            error: function() {
                $('#' + targetFilenameId).val(docId);
    ...
    $(document).ready(function () {
        // Resolve tên file Google Sheets khi tải trang
        var ggsheetId = $('#ggsheet_id').val();
        if (ggsheetId) {
            fetchSpreadsheetTitle(ggsheetId, 'ggsheet_filename');
        }
        var ggsheet2Id = $('#ggsheet2_id').val();
        if (ggsheet2Id) {
            fetchSpreadsheetTitle(ggsheet2Id, 'ggsheet2_filename');
        }

        // Gắn sự kiện click cho các nút chọn file
        $('.btn-select-sheet').on('click', function() {
            var targetId = $(this).data('target-id');
            var targetFilenameId = $(this).data('target-filename-id');
            ...
            createPicker(targetId, targetFilenameId, targetName);
        });
